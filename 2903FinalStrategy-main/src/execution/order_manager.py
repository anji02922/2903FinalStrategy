            ...
            try:
                ...
                for order in remaining:
                    try:
                        ...
                        pass
                # Bulk-cancel any remaining algo/conditional orders
                try:
                    symbol_id = self.client.symbol.replace("/", "").replace(":USDT", "")
                    self.client.exchange.fapiPrivateDeleteAllOpenOrders({"symbol": symbol_id})
                except Exception:
                    pass
                try:
                    symbol_id = self.client.symbol.replace("/", "").replace(":USDT", "")
