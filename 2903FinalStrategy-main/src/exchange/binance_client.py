...
class BinanceClient:
    ...
    def cancel_all_orders(self):
        ...
        # Binance USDM symbol format: "ETHUSDT" (no slash, no colon).
        try:
            symbol_id = self.symbol.replace("/", "").replace(":USDT", "")
            self.exchange.fapiPrivateDeleteAllOpenOrders({"symbol": symbol_id})
        except Exception as e:
            logger.debug(f"Cancel all open orders (fapi): {e}")
        try:
            symbol_id = self.symbol.replace("/", "").replace(":USDT", "")
    ...
    def fetch_open_orders(self):
        ...
        try:
            ...
            logger.debug(f"Fetch algo orders: {e}")
        return regular
